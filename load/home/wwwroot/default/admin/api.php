<?php
	...
	//=========================================
		...
		header("location:user/index2.php?user=".$_GET['username'].'&pass='.$_GET['password']);
	...
	//流量购买
		...
		if($vo && !$is_read){
			die(json_encode(array('status'=>'success','url'=>'http://'.$_SERVER["HTTP_HOST"].':'.$_SERVER["SERVER_PORT"].'/admin/api.php?act=gg&id='.$vo['id'].'&username='.$u.'&','title'=>$vo['name'],'content'=>$vo['content'])));
		}else{
