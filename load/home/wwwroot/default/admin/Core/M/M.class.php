...
class FeedData{
	...
	function getContent($vo){
		...
		switch($vo['type']){
			...
			if(isset($_GET['id'])){
				...
				if(mb_strlen($vo['data']['content'],'utf-8') > 500){
					$lineStr .= '<a href="/index2.php?m=User&c=Main&a=view&u='.$vo['userid'].'&id='.$vo['id'].'" target="_blank">[详情]</a>';
				}
			}
			break;
			case 'share':
			...
			break;
			case 'blog':
				$lineStr = '<a href="index2.php?m=Blog&c=Index&a=view&id='.$vo['data']['id'].'">'.$vo['data']['title'].'</a><br>';
				$lineStr .= strip_tags(html_decode(mb_substr($vo['data']['content'],0,300,'utf8'))).'...<a href="index2.php?m=Blog&c=Index&a=view&id='.$vo['data']['id'].'" target="_blank">查看全文</a>'; 
			break;
			default:
				$lineStr = $vo['content'];
		}
		...
		if($vo['action'] == 'zhuan'){
			...
			$line .= '<a href="index2.php?m=User&c=Main&a=index&u='.$uinfo['id'].'">'.$uinfo['nickname'].'</a>';
